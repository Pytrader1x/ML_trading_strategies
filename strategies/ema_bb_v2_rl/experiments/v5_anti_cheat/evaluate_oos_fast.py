#!/usr/bin/env python3
"""
Fast OOS Evaluation for v5_anti_cheat.

KEY CHANGES from v4:
1. TRAIL_BE masking: Blocked until min_trail_bar (bar 1)
2. True breakeven: 0 buffer (not 0.1 ATR)
3. Min profit for TRAIL: Must have 2 pips profit to activate breakeven
4. Enhanced anti-cheat validation metrics
"""
...
def get_action_mask(bar: int, min_exit_bar: int, min_trail_bar: int):
    ...
    V5: Now also masks TRAIL_BE until min_trail_bar.
    """
    ...
    # V5: Block TRAIL_BE before min_trail_bar
...
def evaluate_episode(policy, episode, device: str, config):
    """Evaluate policy on single episode with v5 anti-cheat.
    ...
    breakeven_buffer = config.reward.breakeven_buffer_pct  # 0 in v5
    ...
    # V5: Track various blocking stats
    ...
    for bar in range(max_bars):
        ...
        # V5: Track early attempts (should be zero with proper masking)
        ...
        if action == Actions.EXIT:
            ...
            # V5: Check minimum profit requirement
        ...
        if bar + 1 < max_bars:
            ...
            if breakeven_active:
                # V5: True breakeven - exit at buffer (0 in v5)
...
def main():
    ...
    print("=" * 70)
    print(" DEEP OOS EVALUATION - v5_anti_cheat")
    ...
    breakeven_rate = ((returns >= -0.0005) & (returns <= 0.0005)).mean()  # Within 5 pips of breakeven
    ...
    breakeven_exits = returns[(returns >= -0.001) & (returns <= 0.001)]
    profit_exits = returns[returns > 0.005]

    print("\n" + "=" * 70)
    print(" OOS RESULTS - v5_anti_cheat (2022-2025)")
    print("=" * 70)
    ...
    print(f"  Breakeven:     {breakeven_rate * 100:.1f}% (within 5 pips)")
    ...
    print(f"    Breakeven-ish:         {len(breakeven_exits)} ({len(breakeven_exits)/len(returns)*100:.1f}%)")
    print(f"    Profit exits (> 0.5%): {len(profit_exits)} ({len(profit_exits)/len(returns)*100:.1f}%)")
    ...
    # V5: Enhanced anti-cheat validation
    print("\n" + "=" * 70)
    print(" V5 ANTI-CHEAT VALIDATION")
    print("=" * 70)
    ...
    # Check 4: Quick exits
    print(f"\n  4. Quick Exits (length <= {config.reward.min_exit_bar} bars):")
    ...
    # Check 6: Sharpe sanity
    print(f"\n  6. Sharpe Ratio Sanity:")
    ...
    if checks_passed:
        ...
        print(" OVERALL: FAIL - Possible issues detected")
    print("=" * 70)
