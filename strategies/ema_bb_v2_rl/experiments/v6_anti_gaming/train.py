#!/usr/bin/env python3
"""
Training script for v6_anti_gaming experiment.

V6 ANTI-GAMING: Fix V5's "breakeven farming" gaming behavior.

KEY CHANGES from v5:
1. ASYMMETRIC BREAKEVEN: Track recovery_exits vs giveback_exits
2. MFE DECAY: Log mfe_decay_applied count
3. TIGHTEN COOLDOWN: Log cooldown_blocks
4. NEW METRICS: recovery_rate, giveback_rate, profit_rate in logging
...
Usage:
    cd experiments/v6_anti_gaming
    python train.py --wandb

    # Or from strategy root:
    python experiments/v6_anti_gaming/train.py --wandb
"""
...
class PPOTrainer:
    """PPO Trainer with v6 anti-gaming configuration.

    KEY v6 ADDITIONS:
    - Asymmetric breakeven rewards: recovery_bonus, giveback_penalty
    - MFE decay penalty: mfe_decay_coef
    - Tighten cooldown: tighten_cooldown
    - Enhanced logging for anti-gaming metrics
    """
    ...
    def __init__(
        ...
        # Initialize W&B
        if self.use_wandb:
            run_name = run_name or f"v6_anti_gaming_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            wandb.init(
                project=config.wandb_project,
                entity=config.wandb_entity,
                name=run_name,
                config={
                    'experiment': 'v6_anti_gaming',
                    'n_envs': config.n_envs,
                    ...
                    'recovery_bonus': config.reward.recovery_bonus,
                    'giveback_penalty': config.reward.giveback_penalty,
                    'breakeven_band': config.reward.breakeven_band,
                    'mfe_decay_coef': config.reward.mfe_decay_coef,
                    'min_mfe_for_decay': config.reward.min_mfe_for_decay,
    ...
    def train(self) -> Dict[str, list]:
        """Main training loop with V6 anti-gaming rewards."""
        config = self.config

        num_updates = config.total_timesteps // (config.n_steps * config.n_envs)

        print(f"\n{'=' * 70}")
        print(f" V6_ANTI_GAMING Training - {config.total_timesteps:,} timesteps")
        print(f"{'=' * 70}")
        ...
        print(f"\n  --- V6 Anti-Gaming Features ---")
        print(f"  recovery_bonus: {config.reward.recovery_bonus} (reward scratch after loss)")
        print(f"  giveback_penalty: {config.reward.giveback_penalty} (penalize scratch after profit)")
        print(f"  breakeven_band: {config.reward.breakeven_band:.4f} (~{config.reward.breakeven_band*10000:.0f} pips)")
        print(f"  mfe_decay_coef: {config.reward.mfe_decay_coef} (penalize MFE giveback)")
        print(f"  min_mfe_for_decay: {config.reward.min_mfe_for_decay:.4f} (~{config.reward.min_mfe_for_decay*10000:.0f} pips)")
    ...
    def _log_progress(
        ...
        """Log training progress with V6 anti-gaming stats."""
        ...
        # V6: Get anti-gaming stats
        anti_gaming_stats = self.env.get_anti_gaming_stats()
        ...
        # V6: Anti-gaming metrics
        print(f"  V6 Anti-Gaming:")
        print(f"    Recovery: {anti_gaming_stats['recovery_exits']} ({anti_gaming_stats['recovery_rate']:.1%})")
        print(f"    Giveback: {anti_gaming_stats['giveback_exits']} ({anti_gaming_stats['giveback_rate']:.1%})")
        print(f"    Profit:   {anti_gaming_stats['profit_exits']} ({anti_gaming_stats['profit_rate']:.1%})")
        print(f"    MFE decay: {anti_gaming_stats['mfe_decay_applied']} | Cooldown blocks: {anti_gaming_stats['cooldown_blocks']}")
        ...
        if self.use_wandb:
            wandb.log({
                ...
                'anticheat/cooldown_blocks': lookahead_stats['cooldown_blocks'],
                ...
                # V6: Anti-gaming stats
                'antigaming/recovery_exits': anti_gaming_stats['recovery_exits'],
                'antigaming/giveback_exits': anti_gaming_stats['giveback_exits'],
                'antigaming/profit_exits': anti_gaming_stats['profit_exits'],
                'antigaming/recovery_rate': anti_gaming_stats['recovery_rate'],
                'antigaming/giveback_rate': anti_gaming_stats['giveback_rate'],
                'antigaming/profit_rate': anti_gaming_stats['profit_rate'],
                'antigaming/mfe_decay_applied': anti_gaming_stats['mfe_decay_applied'],
                'antigaming/avg_recovery_bonus': anti_gaming_stats['avg_recovery_bonus'],
                'antigaming/avg_giveback_penalty': anti_gaming_stats['avg_giveback_penalty'],
                'antigaming/avg_mfe_decay_penalty': anti_gaming_stats['avg_mfe_decay_penalty'],
                # Action distribution
        ...
        # V6: Warnings for gaming behavior
        if anti_gaming_stats['giveback_rate'] > 0.5:
            print(f"  WARNING: Giveback rate {anti_gaming_stats['giveback_rate']:.1%} > 50% - model may be gaming!")
        if anti_gaming_stats['profit_rate'] < 0.1:
            print(f"  WARNING: Profit rate {anti_gaming_stats['profit_rate']:.1%} < 10% - model not letting winners run!")
        if mean_length < 5:
    ...
    def _save_checkpoint(self, update: int, final: bool = False):
        ...
        torch.save({
            ...
            # V6: Save anti-gaming parameters
            'recovery_bonus': self.config.reward.recovery_bonus,
            'giveback_penalty': self.config.reward.giveback_penalty,
            'mfe_decay_coef': self.config.reward.mfe_decay_coef,
            'tighten_cooldown': self.config.reward.tighten_cooldown,
        }, path)
...
def evaluate_policy(
    ...
    """Evaluate trained policy with V6 anti-gaming metrics."""
    policy.eval()
    ...
    # Reset env stats for clean evaluation
    env.reset_stats()
    ...
    # V6: Get anti-gaming stats from evaluation
    anti_gaming_stats = env.get_anti_gaming_stats()
    ...
    metrics = {
        ...
        # V6: Anti-gaming metrics
        'recovery_rate': anti_gaming_stats['recovery_rate'],
        'giveback_rate': anti_gaming_stats['giveback_rate'],
        'profit_rate': anti_gaming_stats['profit_rate'],
    }

    print("\n" + "=" * 70)
    print(" EVALUATION RESULTS - v6_anti_gaming")
    print("=" * 70)
    ...
    print(f"\n  V6 Exit Analysis:")
    print(f"    Recovery exits (good): {anti_gaming_stats['recovery_exits']} ({metrics['recovery_rate']*100:.1f}%)")
    print(f"    Giveback exits (bad):  {anti_gaming_stats['giveback_exits']} ({metrics['giveback_rate']*100:.1f}%)")
    print(f"    Profit exits (good):   {anti_gaming_stats['profit_exits']} ({metrics['profit_rate']*100:.1f}%)")
    print(f"    MFE decay applied:     {anti_gaming_stats['mfe_decay_applied']}")
    print(f"    Cooldown blocks:       {anti_gaming_stats['cooldown_blocks']}")
    print(f"\n  Action Distribution:")
    ...
    if metrics['giveback_rate'] > 0.4:
        print(f"\n  WARNING: Giveback rate {metrics['giveback_rate']*100:.1f}% > 40% - model may still be gaming!")
    if metrics['profit_rate'] < 0.2:
        print(f"\n  WARNING: Profit rate {metrics['profit_rate']*100:.1f}% < 20% - model not letting winners run!")

    return metrics


def main():
    parser = argparse.ArgumentParser(description="Train v6_anti_gaming RL Exit Policy")
