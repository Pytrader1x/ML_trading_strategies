"""
Vectorized GPU Environment for v3_balanced_exits experiment.

KEY CHANGES from v2:
1. BOUNDED counterfactual window (8 bars vs 20)
2. ASYMMETRIC coefficients (defensive > regret)
3. NO hold bonus (removed free +0.5/step)
4. EXIT bonus for decisive action
5. PROGRESSIVE time pressure (quadratic growth)
...
class VectorizedExitEnv:
    """
    Vectorized environment for v3_balanced_exits.

    KEY DIFFERENCES from v2:
    - BOUNDED counterfactual window (8 bars)
    - ASYMMETRIC coefficients (defensive_coef > regret_coef)
    - NO hold bonus (was +0.5/step in v2)
    - EXIT bonus for decisive action
    - PROGRESSIVE time penalty (quadratic growth)
    """
    ...
    def __init__(
        ...
        # Action distribution tracking for v3
    ...
    def _compute_counterfactual_reward(
        ...
        v3 CHANGES:
        1. BOUNDED window (8 bars vs 20)
        2. ASYMMETRIC coefficients (defensive_coef > regret_coef)
        3. EXIT bonus for decisive action
        4. NO exit cost (removed)

        Returns:
            Additional reward adjustment for exiting environments
        """
        cfg = self.reward_config
        lookforward = cfg.counterfactual_window  # 8 bars (bounded)
        ...
        for idx in exit_indices:
            ...
            # BOUNDED to 8 bars (was 20 in v2)
            ...
            if end_bar > bar:
                ...
                if len(future_pnls) > 0:
                    ...
                    # ONLY APPLY IF IN PROFIT - prevents gaming by exiting at 0 to collect bonus
                    # This fixes the EXIT collapse issue
                    avoided_loss = max(0, pnl - future_worst)
                    if pnl > 0:
                        defensive_bonus = avoided_loss * cfg.defensive_coef
                    else:
                        defensive_bonus = 0.0  # No defensive bonus for exits at loss

                    # Opportunity cost: penalize if we missed future gains
                    # REDUCED: regret_coef = 0.3 (gentle, not crushing)
                    missed_gain = max(0, future_best - pnl)
                    opportunity_cost = missed_gain * cfg.regret_coef

                    # Net counterfactual adjustment (SCALED)
            ...
            # EXIT BONUS for decisive action (NEW in v3)
            # Small positive reward for taking any exit action
            counterfactual_reward[idx] += cfg.exit_bonus * cfg.reward_scale

            # NO exit cost (removed in v3 - was -0.0001 in v2)
    ...
    def step(
        ...
        """Execute actions with balanced counterfactual rewards."""
        ...
        # ================================================================
        # HOLD action (0) - NO bonus (removed in v3)
        # ================================================================
        hold_mask = (actions == Actions.HOLD) & self.position_open
        # v3: NO hold bonus - removed the +0.5 per hold that caused collapse
        # The only reward for HOLD is through the differential Sharpe in _shape_reward

        # ================================================================
        # EXIT action (1) - Uses balanced counterfactual rewards
        # ================================================================
        ...
        if exit_mask.any():
            ...
            # Counterfactual adjustment (BALANCED: defensive > opportunity cost)
        ...
        # ================================================================
        # PARTIAL_EXIT (4) - Uses balanced counterfactual rewards
        # ================================================================
    ...
    def _shape_reward(
        ...
        v3 CHANGES:
        - RESTORED time_coef (0.005 vs v2's 0.002)
        - PROGRESSIVE time penalty (quadratic growth)
        """
        ...
        # Time Decay - RESTORED and PROGRESSIVE (v3)
        # v2 used 0.002 (too low), v3 uses 0.005 (v1 value)
        # PROGRESSIVE: quadratic growth to create increasing urgency
        bars = self.current_bar.float()
        bars_norm = (bars / 200.0).clamp(max=1.0)  # Normalize to [0, 1]

        # Quadratic time penalty: grows slowly at first, faster later
        # Bar 50: 0.005 * (50/200)^2 = 0.0003
        # Bar 100: 0.005 * (100/200)^2 = 0.00125
        # Bar 150: 0.005 * (150/200)^2 = 0.0028
