#!/usr/bin/env python3
"""
PPO Training Script for RL Exit Optimizer.
...
class PPOTrainer:
    """
    PPO Trainer with entropy annealing and W&B logging.
    ...
    def __init__(
        ...
        # Initialize W&B
        if self.use_wandb:
            run_name = run_name or f"ppo_exit_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            wandb.init(
    ...
    def train(self) -> Dict[str, list]:
        ...
        Returns dict of training metrics history.
        """
        config = self.config
    ...
    def _save_checkpoint(self, update: int, final: bool = False):
        ...
        if final:
            ...
            path = models_dir / f"exit_policy_{update:06d}.pt"
...
def main():
    ...
    # Load episodes
    episode_file = Path(args.episodes)
